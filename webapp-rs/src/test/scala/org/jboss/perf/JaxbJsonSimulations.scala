...
object JaxbJsonSimulations {
  ...
  class Post extends BaseSimulation with AppJson {
    def run(http: Http) = {
      http.post("/jaxb").body(StringBody(JaxbPerson.JOHNNY_JSON)).check(status.is(204));
    }
  }

  class Put extends BaseSimulation with AppJson {
    def run(http: Http) = {
      http.put("/jaxb").body(StringBody(JaxbPerson.JOHNNY_JSON)).check(status.is(200), bodyString.is(JaxbPerson.JOHNNY_JSON) );
    }
  }
}
